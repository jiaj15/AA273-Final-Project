...
%     B = Z(stateDim*(2*d-1)+1:2*stateDim*d,:);
    Z(stateDim*(2*d-1)+1:2*stateDim*d,:) = [repmat(D(:,d),1,q); repmat([0;0],1,q)];           %???
end
...
sigma0 = diag([0.4, 0.6, 3, 3, 0.4, 0.6, 3, 3]);
...
sig = diag([0.1, 0.1, 0.1, 0.1]);         % small sigma in MRD model
Vn = [2, 1; 1, 5] / 10; 
ita = diag([1, 1])/50;             % diagonal matrix of mean reversion strengths in each spacial dimention, s*s
rho = diag([1,1])/50;              % diagonal matrix of drag coeff, s*s
r = stateDim;
...
for t=1:1:N
    ...
    for interval = 1:1:RATIO
        [x_next, y, R_t, U_t, G_til, m_til] = ERV(x, trueDes, h, sig, Vn, r, Tq, tn,  ita, rho);
        tn = tn + h;
        x = x_next;
    end
    ...
    for d=1:1:desNum
        ...
        for i=1:1:q
            ...
            [~, ~, R_t, U_t, G_til, m_til] = ERV(z(1:stateDim,1), pd, measure_t, sig, Vn, r, Ti, tn,  ita, rho);
...
if Pred_flag
    for idx = 1 : Pred_steps
        St_predpd{idx} = predTraj(Z, Sigma, h, sig, Vn, r, quadPoints, tn, desNum, q, idx, u_id, D, stateDim,  ita, rho);
    end
end
